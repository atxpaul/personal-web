...
export const saveSiteConfig = async (locale, configData, auth = null) => {
  try {
    ...
    if (auth && !auth.currentUser) {
      throw new Error('Debes estar autenticado para guardar la configuración')
    }
    ...
    if (error.code === 'permission-denied') {
      throw new Error('No tienes permisos para guardar. Verifica que estés autenticado como admin y que el documento /site/admins exista con tu UID.')
    }
    throw error
  }
}
...
export const saveAboutMe = async (locale, aboutMeData, auth = null) => {
  try {
    // Verificar que el usuario esté autenticado
    if (auth && !auth.currentUser) {
      throw new Error('Debes estar autenticado para guardar la información')
    }
 ...
 * @returns {Promise<void>}
 */
export const saveSkills = async (skillsData) => {
  try {
    const docRef = doc(db, 'site', 'skills')
    const docSnap = await getDoc(docRef)
 ...
 * @returns {Promise<void>}
 */
export const saveProject = async (projectId, locale, projectData, commonData = {}, auth = null) => {
  try {
    // Verificar que el usuario esté autenticado
    if (auth && !auth.currentUser) {
    ...
    if (error.code === 'permission-denied') {
      throw new Error('No tienes permisos para guardar. Verifica que estés autenticado como admin.')
    }
    throw error
  }
}
...
export const deleteProject = async (projectId, auth = null) => {
  try {
    // Verificar que el usuario esté autenticado
    if (auth && !auth.currentUser) {
      throw new Error('Debes estar autenticado para eliminar el proyecto')
    }
    ...
    if (error.code === 'permission-denied') {
      throw new Error('No tienes permisos para eliminar. Verifica que estés autenticado como admin.')
    }
    throw error
  }
}
