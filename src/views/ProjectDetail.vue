<template>
  <main class="flex-grow flex flex-col items-center py-8 px-4 md:px-10 lg:px-20">
    <!-- Breadcrumb -->
    <div class="w-full max-w-6xl mb-6 flex items-center gap-2 font-mono text-sm text-gray-500">
      <router-link 
        to="/projects" 
    ...
    <!-- Project Detail Card -->
    <div v-if="project" class="w-full max-w-6xl bg-console-bg border border-console-border rounded-lg shadow-2xl overflow-hidden flex flex-col mb-16 relative group">
      <!-- Header -->
      <div class="bg-[#161b22] border-b border-console-border flex items-center justify-between px-4 py-2 select-none">
      ...
      <!-- Content -->
      <div class="flex flex-col md:flex-row overflow-x-auto min-h-[600px] relative">
        <!-- Line Numbers -->
        <div class="hidden md:flex flex-col items-end px-3 py-6 bg-console-bg border-r border-console-border text-gray-600 font-mono text-sm leading-6 select-none min-w-[3rem]">
          <span v-for="n in 40" :key="n">{{ n }}</span>
        </div>

        <!-- Code Content -->
        <div class="flex-1 p-4 md:p-6 font-mono text-sm md:text-base leading-6 text-gray-300 overflow-x-auto">
          <div class="space-y-1">
            ...
            <div>
              ...
              <span class="text-accent">{{ project.version || '1.0.0' }}</span>
            </div>
            <div>
            ...
            <div v-if="project.providerFull || project.provider">
              ...
              <span class="text-secondary">"{{ project.providerFull || project.provider }}"</span>
            </div>
            <div class="h-4"></div>
            
            <div>
              <span class="text-syntax-key">summary</span>: <span class="text-syntax-string">|</span>
            </div>
            <div class="pl-4 text-gray-400 border-l-2 border-console-border ml-1 mb-2 whitespace-pre-line">
            ...
            <div class="pl-4 grid grid-cols-1 md:grid-cols-2 gap-2 mt-2 max-w-2xl">
              <div 
              ...
              >
                ...
                <span class="material-symbols-outlined text-[16px] text-primary">{{ tech.icon || 'dns' }}</span>
                <span class="text-syntax-string">{{ tech.name }}</span>
                <span v-if="tech.comment" class="text-syntax-comment text-xs italic opacity-0 group-hover:opacity-100">
                  // {{ tech.comment }}
                </span>
              </div>
            </div>
            ...
            <div v-if="project.architecture">
              <span class="text-syntax-key">architecture</span>: <span class="text-syntax-string">|-</span>
            </div>
            <div v-if="project.architecture" class="pl-4 mt-2 overflow-x-auto pb-4">
              <pre class="architecture-diagram text-blue-400 select-none m-0 p-0">{{ architectureText }}</pre>
            </div>
            
            <div class="h-4"></div>
            
            <div>
              <span class="text-syntax-key">links</span>:
            </div>
            <div class="pl-4 mt-2 flex flex-wrap gap-4">
              ...
              >
                <span class="absolute inset-0 bg-primary/10 opacity-0 group-hover:opacity-100 transition-opacity rounded"></span>
            ...
            <div class="h-4"></div>
            <div class="blinking-cursor text-primary">$ <span class="text-gray-500">_</span></div>
          </div>
        </div>
      </div>

      <!-- Footer Status Bar -->
      <div class="bg-primary text-white text-xs font-mono py-1 px-4 flex justify-between items-center z-10">
        <div class="flex gap-4 font-bold">
          <span>NORMAL</span>
          <span class="font-normal text-white/80">{{ project.id }}.yml</span>
        </div>
    ...
    <!-- Loading State -->
    <div v-if="loading" class="w-full max-w-6xl text-center text-gray-400 py-12">
      <p>Loading project...</p>
    </div>

    <!-- Error State -->
    <div v-if="error" class="w-full max-w-6xl text-center text-red-400 py-12">
      <p>{{ error }}</p>
    </div>

    <!-- Related Projects -->
    <div v-if="relatedProjects.length > 0" class="w-full max-w-6xl">
      <div class="flex items-center gap-3 mb-6 border-b border-console-border pb-2">
        <span class="material-symbols-outlined text-primary">history</span>
        <h3 class="text-xl font-bold text-white font-mono">Related Logs</h3>
        <span class="text-gray-600 font-mono text-sm ml-auto">ls -t | head -n {{ relatedProjects.length }}</span>
      </div>
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div class="p-2 bg-[#161b22] rounded-lg border border-console-border group-hover:border-primary/30 text-white">
              <span class="material-symbols-outlined">{{ related.icon || 'folder' }}</span>
            </div>
            <span class="text-xs font-mono text-gray-500">{{ formatDate(related.updatedAt) }}</span>
          </div>
          <h4 
...
const techStackList = computed(() => {
  ...
  return project.value.technologies?.map(tech => ({
    name: tech,
    icon: 'dns',
    comment: null
  })) || []
